...
export var disguises_duration = 5 # How long a disguises can last
export var disguises_slowdown = 0.25
...
func _ready():
	...
	vision_mode = DARK
	collision_layer = 16
	$Timer.wait_time = disguises_duration
...
func cycle_vision_mode():
	if vision_mode == DARK:
		get_tree().call_group("interface", "NightVision_mode_on")
		vision_mode = NIGHTVISION
	elif vision_mode == NIGHTVISION:
		get_tree().call_group("interface", "DarkVision_mode_on")
		vision_mode = DARK
...
func disguise():
	...
	velocity_multiplier = disguises_slowdown
	$Timer.start()
...
func get_briefcase():
	return has_briefcase	

func collect_briefcase():
	var loot = Node.new()
	loot.set_name("briefcase")
	add_child(loot)
	get_tree().call_group("LootDisplay", "collect_loot")
